                  ...
                  {passengers.map((p, i) => (
                    <div key={i} className="flex items-center justify-between bg-gray-50 dark:bg-slate-700/50 p-3 rounded-lg border border-gray-100 dark:border-slate-600">
                      <div>
                        ...
                        <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-1">Pickup: {renderPickupPoint(p.pickup, true)}</div>
                      </div>
                      <button 
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400 mt-0.5 flex flex-wrap items-center gap-1.5">
                          {renderPickupPoint(p.pickup_point, true)}
                          <span>·</span>
                          <span className="font-semibold text-blue-600 dark:text-blue-400">Priority {p.priority}</span>
                        </div>
                      </div>
                    </div>
...
export default function DriverDashboard() {
  ...
  const [driverGatePasses, setDriverGatePasses] = useState([]);
  ...
  const fetchDriverGatePasses = async () => {
    if (!myVehicle) return;
    setLoadingGoods(true);
    try {
      const res = await axios.get(`${API}/gate-passes?vehicle_no=${myVehicle.vehicle_number}&vehicle_id=${myVehicle.id}`);
      setDriverGatePasses(res.data || []);
    } catch (error) {
      console.error('Failed to fetch driver gate passes', error);
    } finally {
  ...
  const renderHome = () => (
    <div className="space-y-6">
      {/* Welcome */}
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-4 sm:p-6 3xl:p-8 text-white shadow-lg shadow-blue-200 dark:shadow-none">
        <div className="flex justify-between items-start">
          <div>
            <p className="text-blue-100 text-sm mb-1">Good {new Date().getHours() < 12 ? 'Morning' : 'Afternoon'} 👋</p>
            <h2 className="text-xl sm:text-2xl 3xl:text-3xl font-bold mb-1">{driver.name || 'Driver'}</h2>
          </div>
          <button 
      ...
      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 3xl:grid-cols-4 gap-3 sm:gap-4">
        {stats.map(s => {
          const Icon = s.icon;
          return (
            <div 
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-black text-gray-900 dark:text-white">Goods on Cab / Gate Passes</h3>
            <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Approved gate passes assigned to your vehicle ({myVehicle?.vehicle_number || 'No Vehicle'}).</p>
          </div>
          <button 
            onClick={fetchDriverGatePasses} 
          ...
          >
            <RefreshCw className={`h-3 w-3 ${loadingGoods ? 'animate-spin' : ''}`} />
            Refresh
          </button>
        </div>
        ...
        ) : driverGatePasses.length === 0 ? (
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 text-center text-gray-500 shadow-sm border border-gray-100 dark:border-slate-700">
            <Box className="h-10 w-10 mx-auto mb-3 opacity-40 text-gray-450" />
            <p className="text-sm font-semibold">No goods scheduled for this vehicle today.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {driverGatePasses.map(gp => (
              <div key={gp.id} className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-gray-150 dark:border-slate-700/60 shadow-sm space-y-4">
                <div className="flex justify-between items-start flex-wrap gap-2">
                  ...
                  <div className="text-right">
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase bg-green-100 text-green-700 dark:bg-green-950/30 dark:text-green-400 border border-green-200 dark:border-green-800">{gp.status}</span>
                    <span className="block text-[10px] font-black text-gray-400 dark:text-slate-500 mt-1 uppercase tracking-wider">{gp.material_type}</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs bg-gray-50/75 dark:bg-slate-900/30 p-3 rounded-xl border border-gray-100 dark:border-slate-800">
                  <p>Dispatched To: <strong className="font-bold text-gray-700 dark:text-slate-350">{gp.dispatched_to}</strong></p>
                  <p>Purpose: <strong className="font-semibold text-gray-750 dark:text-slate-350">{gp.purpose || '—'}</strong></p>
                  {gp.invoice_dc_no && <p>Invoice/DC: <strong className="font-semibold text-gray-750 dark:text-slate-300">{gp.invoice_dc_no}</strong></p>}
                  {gp.expected_return_date && <p>Expected Return: <strong className="font-semibold text-amber-600 dark:text-amber-500">{new Date(gp.expected_return_date).toLocaleDateString('en-IN')}</strong></p>}
                </div>

                <div className="space-y-2">
                  <p className="text-xs font-bold text-gray-450 dark:text-slate-400 uppercase tracking-wider">Materials Breakdown:</p>
                  <div className="overflow-hidden rounded-xl border border-gray-150 dark:border-slate-700 bg-white dark:bg-slate-900/10 text-xs">
                    <table className="w-full text-left">
                      <thead className="bg-gray-50 dark:bg-slate-800 text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">
                        <tr>
                      ...
                      <tbody className="divide-y divide-gray-100 dark:divide-slate-800 font-medium">
                        {gp.gate_pass_materials?.map((mat, mIdx) => (
                          <tr key={mat.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-850/50">
                            ...
                            <td className="px-3 py-2 text-gray-650 dark:text-slate-450">{mat.uom}</td>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto w-full relative">
        <div className="max-w-4xl 3xl:max-w-6xl mx-auto px-4 py-6 md:py-8 pb-28 md:pb-8">
